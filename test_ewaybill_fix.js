...
async function testEWayBillGeneration() {
  ...
  try {
    ...
    if (eWayBills.length > 0) {
      ...
      if (isValidDate(sampleBill.ewayBillDate) && isValidDate(sampleBill.validUpto)) {
        console.log('✅ E-Way Bill dates are properly formatted as Date objects');
      } else {
        console.log('❌ E-Way Bill dates are not properly formatted');
      }
    } else {
      console.log('No E-Way Bills generated (this is normal for small test data)');
    }
