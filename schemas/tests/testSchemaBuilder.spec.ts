...
test('Field Counts', function (t) {
  ...
  t.equal(regionalSchemaMap.Party.fields?.length, 8);
  t.end();
});

test('Quick Edit Field Counts', function (t) {
  t.equal(appSchemaMap.Party.quickEditFields?.length, 5);
  t.equal(regionalSchemaMap.Party.quickEditFields?.length, 14);
  t.end();
});
...
test('Field Counts', function (t) {
  t.equal(regionalCombined.Party.fields?.length, 17);
  t.end();
});
...
test('Field Counts', function (t) {
  t.equal(abstractCombined.Customer!.fields?.length, 17);
  t.end();
});

test('Quick Edit Field Counts', function (t) {
  t.equal(abstractCombined.Customer!.quickEditFields?.length, 14);
  t.end();
});
